...
export default function IncidentReports() {
  ...
  const fetchIncidents = async () => {
    try {
      ...
      setPagination(prev => ({ ...prev, total: sampleIncidents.length, pages: 1 }));
  ...
  const handleDownloadPdf = async (incidentId: number, e?: React.MouseEvent) => {
    ...
    try {
      ...
      if (incidentId === 1) {
        ...
        toast.success('Downloading sample incident report');
      } else {
  ...
  // Filter sample data locally when API is down
  const filteredIncidents = apiError 
