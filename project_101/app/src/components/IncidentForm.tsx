 ...
 * Handles new incident submission with DOCX template support
 */

import { useState } from 'react';
import { incidentsApi } from '@/services/api';
import { Calendar, Clock, MapPin, FileText, Upload, X, Loader2, Download, FileSpreadsheet } from 'lucide-react';
...
export default function IncidentForm({ onSuccess, onCancel }: IncidentFormProps) {
  ...
  const validateForm = () => {
    // If a document is uploaded, bypass text validation
    if (file) {
      setErrors({});
      return true;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm()) {
      toast.error('Please fill in all required fields or upload the completed DOCX form.');
      return;
    }
  ...
  const downloadTemplate = () => {
    ...
    link.href = '/templates/Incident_form.docx';
    link.download = 'DMMMSU-SLUC_Incident_Form.docx';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast.info('Downloading incident form template...');
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      {/* Template Download Banner */}
      <div className="p-4 rounded-lg bg-gradient-to-r from-blue-600/20 to-indigo-600/20 border border-blue-500/30">
        <div className="flex items-start gap-3">
          <FileSpreadsheet className="w-5 h-5 text-blue-400 mt-0.5" />
          <div className="flex-1">
            <h4 className="text-sm font-medium text-blue-300">Official Incident Form Template</h4>
            <p className="text-xs text-blue-400/80 mt-1">
              Download and fill out the official DOCX template for detailed incident reporting.
            </p>
          </div>
          <Button
            type="button"
            ...
            className="border-blue-500/50 text-blue-300 hover:bg-blue-500/20 hover:text-blue-200"
          >
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input
            id="cause"
            type="text"
            placeholder="e.g., Fire, Flood, Electrical Issue, Typhoon"
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-slate-500 mb-2">
          Upload the completed DOCX form or any supporting files (images, PDFs)
        </p>
        <div
        ...
        >
          {file ? (
            ...
            <label className="cursor-pointer">
              ...
              <p className="text-slate-500 text-xs">
                Supports: DOCX, Images, PDFs (max 10MB)
              </p>
              <input
                type="file"
                onChange={handleFileChange}
                className="hidden"
                accept=".docx,image/*,.pdf,.doc"
              />
            </label>
          )}
        </div>
