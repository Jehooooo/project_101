...
import {
  ...
  RefreshCw, Loader2, FileText, CheckCircle2, X, Clock, AlertCircle, Trash2
} from 'lucide-react';
...
/* Sample incident data for demo when API is unavailable
const sampleIncidents: Incident[] = [
  {
    ...
    description: 'As Typhoon Uwan approached Northern Luzon, the province of La Union activated its Incident Command System...',
    status: 'Solved',
    reported_by: 2,
...
export default function IncidentReports() {
  ...
  const fetchIncidents = async () => {
    ...
    setIncidents([]); // empty array if API fails
    setPagination(prev => ({ ...prev, total: 0, pages: 0 }));
    toast.error('Failed to fetch incidents from the server');
  } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDownloadPdf = async (incidentId: number, e?: React.MouseEvent) => {
    ...
    try {
      ...
      if (incidentId === 1) {
        ...
        link.download = 'DMMMSU-Incident-Report-Typhoon-Uwan.pdf';
  ...
  const handleDeleteIncident = async (incidentId: number, e?: React.MouseEvent) => {
    e?.stopPropagation();
...
// Filter incidents from backend based on filters
const filteredIncidents = incidents.filter(inc => {
  if (filters.status && inc.status !== filters.status) return false;
  if (filters.search && !inc.location.toLowerCase().includes(filters.search.toLowerCase()) &&
      !inc.cause.toLowerCase().includes(filters.search.toLowerCase())) return false;
  return true;
});
      ...
      {apiError && (
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-400/80">{apiError}</p>
          </div>
        </motion.div>
      )}
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-white text-lg">
              All Incidents <span className="ml-2 text-sm text-slate-400">({filteredIncidents.length} total)</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
                      ...
                      {filteredIncidents.map((incident, index) => (
                        <motion.tr key={incident.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                          ...
                          <td className="py-3 px-4 text-right">
                            <div className="flex items-center justify-end gap-1 relative z-50">
                              <Button variant="ghost" size="sm"
                                onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleViewDetail(incident); }}
                                className="text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 pointer-events-auto">
                                <Eye className="w-4 h-4" />
                              </Button>
                              <Button variant="ghost" size="sm"
                                onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDownloadPdf(incident.id, e); }}
                                className="text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 pointer-events-auto">
                                <Download className="w-4 h-4" />
                              </Button>
                              <Button variant="ghost" size="sm"
                                onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeleteIncident(incident.id, e); }}
                                className="text-slate-400 hover:text-red-400 hover:bg-red-500/10 pointer-events-auto">
            ...
            <>
              ...
              <div className="space-y-6 mt-4">
                ...
                <div>
                  ...
                  <div className="p-4 rounded-lg bg-slate-700/50 text-slate-300 text-sm whitespace-pre-wrap max-h-60 overflow-y-auto">
